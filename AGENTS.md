...
- **Book tutor** (`/dashboard/library/learn`) parses PDF/EPUB/DOCX in memory (no tokens) and discards the original file. Mongo stores only the compact lesson path (not the PDF). Pipeline: reconstruct chapters (drop TOC/front matter) → analyzer → curriculum architect → typed tutor steps (introduction/explanation/example/practice). Text PDFs up to ~4,000 pages / 80 MB; scanned image PDFs will fail. Set `OPENAI_API_KEY` for richer lessons and grading. Default model: `OPENAI_MODEL=gpt-4o-mini`.
