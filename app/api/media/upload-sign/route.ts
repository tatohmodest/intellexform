...
import {
  ...
  signBrandMediaUpload,
  type BrandUploadKind,
} from '@/lib/cloudinary';

export const dynamic = 'force-dynamic';

const KINDS: BrandUploadKind[] = ['avatar', 'logo', 'cover', 'course_image', 'book_cover'];
const LEARNER_KINDS: BrandUploadKind[] = ['avatar', 'book_cover'];
const ADMIN_KINDS: BrandUploadKind[] = ['logo', 'cover', 'course_image', 'book_cover', 'avatar'];
...
export async function POST(req: NextRequest) {
  ...
  const kind = String(body.kind ?? '') as BrandUploadKind;
  ...
  try {
    const signed = signBrandMediaUpload({ kind, ownerId });
