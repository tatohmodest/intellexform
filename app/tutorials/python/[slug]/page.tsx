import { notFound } from 'next/navigation';
import TutorialLessonView from '@/components/tutorials/TutorialLessonView';
import {
  getAllPythonLessons,
  getPythonLessonNav,
  pythonTutorial,
} from '@/lib/tutorials/python';

export function generateStaticParams() {
  return getAllPythonLessons().map((lesson) => ({ slug: lesson.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }) {
  const nav = getPythonLessonNav(params.slug);
  if (!nav) return { title: 'Lesson not found — Intellex' };
  return {
    title: `${nav.lesson.title} — Python Tutorial | Intellex`,
    description: nav.lesson.description,
  };
}

export default function PythonLessonPage({ params }: { params: { slug: string } }) {
  const nav = getPythonLessonNav(params.slug);
  if (!nav) notFound();

  return (
    <TutorialLessonView
      course={pythonTutorial}
      lesson={nav.lesson}
      prev={nav.prev}
      next={nav.next}
      index={nav.index}
    />
  );
}
