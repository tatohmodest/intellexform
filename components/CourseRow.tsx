      ...
      <div className="mb-3.5 flex items-end justify-between gap-3 sm:mb-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <h3 className="font-display text-[19px] leading-tight sm:text-[24px]">{title}</h3>
          </div>
          {subtitle && <p className="mt-0.5 text-[13px] sm:text-[13.5px]" style={{ color: 'var(--ink-soft)' }}>{subtitle}</p>}
        </div>
        <div className="flex shrink-0 items-center gap-2">
          {href && (
            <Link href={href} className="whitespace-nowrap text-[12.5px] font-semibold sm:text-[13px]" style={{ color: 'var(--green-deep)' }}>
              See all
            </Link>
          )}
          <div className="hidden gap-2 sm:flex">
            <button type="button" onClick={() => scroll(-1)} aria-label="Scroll left" className="flex h-9 w-9 items-center justify-center rounded-full border transition-colors hover:bg-[var(--paper-dim)]" style={{ borderColor: 'var(--line)' }}>
              <ChevronLeft size={18} />
            </button>
            <button type="button" onClick={() => scroll(1)} aria-label="Scroll right" className="flex h-9 w-9 items-center justify-center rounded-full border transition-colors hover:bg-[var(--paper-dim)]" style={{ borderColor: 'var(--line)' }}>
      ...
      <div className="relative">
        ...
        >
          {courses.map((c) => (
            <div
              key={c.slug}
              className="w-[min(72vw,248px)] flex-shrink-0 snap-start sm:w-[250px]"
            >
